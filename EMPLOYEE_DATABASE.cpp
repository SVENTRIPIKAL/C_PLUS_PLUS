/*  EMPLOYEE DATABASE
*   
*   Program reads employee data from 'employees.txt'
...
int main()
{
    ...
    if ( !(INFS.is_open()) )  { cout << "*READ UNSUCCESSFUL*" << endl ; return 1 ; }
                                       ...
                                       const int xEMPLOYEES )
{
    ...
    {
        if ( (i == (xEMPLOYEES - 1)) && (xNAME != xNames.at(i)) )
        {
            i = 0 ;
            cout << "\n*NAME NOT FOUND...TRY AGAIN*" << endl << endl ; 

            cout << "\nEnter Employee Name: " ;   cin >> xNAME ;
        }

        if ( xNAME != xNames.at(i) )        { continue ; }

        else if ( xNAME == xNames.at(i) )     { break ; }
    }
}
...
// returns index of highest average score of all employees
int returnIndexHighestAVERAGE( vector<string> xNames, vector<int> xReview1,
                               vector<int> xReview2,  vector<int> xReview3,
                               const int xEMPLOYEES,  const int xREVIEWS )
{
    ...
    double average ;        double highest ;           string strHIGHEST ;
